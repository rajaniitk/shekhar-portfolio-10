document.addEventListener('DOMContentLoaded', function() {
    ...
    function getStoredDatasets() {
        return [
            { id: 1, name: 'Customer Dataset', rows: 5000, columns: 15 },
            { id: 2, name: 'Sales Data', rows: 10000, columns: 20 },
            { id: 3, name: 'Product Reviews', rows: 8000, columns: 12 }
        ];
    }
    ...
    function getMockFeatures() {
        return [
            { name: 'age', type: 'numeric', values: Array.from({length: 100}, () => Math.floor(Math.random() * 50) + 20) },
            { name: 'income', type: 'numeric', values: Array.from({length: 100}, () => Math.floor(Math.random() * 100000) + 30000) },
            { name: 'score', type: 'numeric', values: Array.from({length: 100}, () => Math.random() * 10) },
            { name: 'category', type: 'categorical', values: Array.from({length: 100}, () => ['A', 'B', 'C', 'D'][Math.floor(Math.random() * 4)]) },
            { name: 'text_review', type: 'text', values: Array.from({length: 100}, () => 'Sample text content for analysis') },
            { name: 'timestamp', type: 'datetime', values: Array.from({length: 100}, () => new Date().toISOString()) }
        ];
    }
